...
// 1. Define your auth type (include 'auto' unless your data source requires explicit auth, like BigQuery, AWS S3, or Azure Blob Storage)
...
class MyConnector implements TabularConnector<MyAuth> {
  ...
  async connect(auth?: MyAuth): Promise<void> {
    ...
    // (like BigQuery, AWS S3, or Azure Blob Storage), throw AuthenticationError when auth is undefined.
